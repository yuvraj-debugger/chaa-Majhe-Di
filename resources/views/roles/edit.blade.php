<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <x-card>
                <x-slot name="header">
                    {{ __('Update Role Details') }}
                </x-slot>

                <form id="role-form" action="{{ route('roles.update', $role) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    
                    <div>
                        <x-label for="title" value="{{ __('Role Title') }}" class="font-bold text-gray-700 mb-1" />
                        <x-input id="title" class="block w-full bg-gray-50/50 border-gray-200 focus:border-chaa-yellow focus:ring-chaa-yellow/20 rounded-xl" type="text" name="title" :value="old('title', $role->title)" placeholder="e.g. Sales Manager, Inventory Admin" required autofocus />
